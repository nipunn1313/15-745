...
//
//===----------------------------------------------------------------------===//
#define LV_NAME "my-loop-vectorize"
...
namespace llvm {
    void initializeMyLoopVectorizePass(PassRegistry&);
}

static cl::opt<unsigned>
VectorizationFactor("my-force-vector-width", cl::init(0), cl::Hidden,
...
struct MyLoopVectorize : public LoopPass {
  static char ID; // Pass identification, replacement for typeid

  MyLoopVectorize() : LoopPass(ID) {
    initializeMyLoopVectorizePass(*PassRegistry::getPassRegistry());
  }
...
char MyLoopVectorize::ID = 0;
static const char lv_name[] = "Loop Vectorization";
INITIALIZE_PASS_BEGIN(MyLoopVectorize, LV_NAME, lv_name, false, false)
INITIALIZE_AG_DEPENDENCY(AliasAnalysis)
INITIALIZE_PASS_DEPENDENCY(ScalarEvolution)
INITIALIZE_PASS_DEPENDENCY(LoopSimplify)
INITIALIZE_PASS_END(MyLoopVectorize, LV_NAME, lv_name, false, false)

//RegisterPass<MyLoopVectorize>  X(LV_NAME, lv_name);

namespace llvm {
  Pass *createMyLoopVectorizePass() {
    return new MyLoopVectorize();
  }
}
